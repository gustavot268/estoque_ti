...
| Necessidade e configuração de limite de requisição (rate limiting) para autenticação, pesquisa e cadastro/edição (Server Actions) | Item 14 da [revisão de segurança](revisao-de-seguranca.md) — a exportação já tem um limite técnico razoável implementado (20 por 5 minutos), mas os demais pontos ainda não têm contrato definido | Segurança da Informação + equipe técnica |
| Confirmação de que o critério de bloqueio de vulnerabilidade no pipeline (Trivy bloqueia HIGH/CRITICAL com correção disponível; `pnpm audit` só reporta, não bloqueia) reflete o apetite de risco da organização | Item 16/17/18 da [revisão de segurança](revisao-de-seguranca.md) — um critério técnico razoável já está implementado e documentado; falta a confirmação formal de que atende ao padrão corporativo | Segurança da Informação |
